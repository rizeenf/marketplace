        ...
        <div className="wrapper flex justify-center items-center flex-col gap-1">
          <figure className=" w-full h-96 overflow-hidden relative cursor-pointer">
            <img
              src={item.img1}
              alt={item.title}
              className=" w-full h-full object-cover absolute z-[2] hover:z-0"
            />
            <img
              src={item.img2}
              alt={item.title}
            ...
            />
            {item.newItem ? (
              <figcaption className="absolute top-1 right-1 bg-gray-400 text-white uppercase p-1 rounded-sm text-sm z-10">
                New item
              </figcaption>
            ) : (
              ""
